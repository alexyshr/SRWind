...
#mync = read_ncdf(ncfile, proxy=TRUE)
...
lon <- seq(from=attr(mync, "dimensions")$longitude$offset, 
           ...
           by = attr(mync, "dimensions")$longitude$delta)
nlon = dim(mync)[1]
...
nhrl_file = paste0("../../ERA5/", "fitted_model_result_all.xlsx")
...
# 2) Gumbel
#nhrl = openxlsx::read.xlsx(xlsxFile=nhrl_file, sheet = "LE_POT_GUM")

# 3) Generalized Pareto - GPA
#nhrl = openxlsx::read.xlsx(xlsxFile=nhrl_file, sheet = "LE_POT_GPA")

# 4) Log Normal - LN3
#nhrl = openxlsx::read.xlsx(xlsxFile=nhrl_file, sheet = "LE_POT_LN3")

# 5) Weibull - WEI
# nhrl = openxlsx::read.xlsx(xlsxFile=nhrl_file, sheet = "LE_POT_WEI")

# 6) Generalized Pareto - GP
# nhrl = openxlsx::read.xlsx(xlsxFile=nhrl_file, sheet = "POT_GP")

# # 7) Yearly Maxima - GEV
#nhrl = openxlsx::read.xlsx(xlsxFile=nhrl_file, sheet = "YM_GEV")
...
era5colpoints = st_as_sf(lonlat.unstacklr.filter, coords=1:2, crs=st_crs(4326), remove=FALSE)

#tiff_file = paste0(input_path, "max_h_10.tif")
tiff_file = paste0(input_path, "cellindexlr.tif")
...
rasterfilesera5 = list.files(path=paste0(input_path, "/h_rasters/h_rasters/"), pattern = "*.*.tif$")
rasterfilesera5 = paste0(input_path, "/h_rasters/h_rasters/", rasterfilesera5)
...
# exel_file <- paste0(output_path, "combined_h_nh_col_pot_pp.xlsx")
...
# #Delete file if it exists
# exel_file <- paste0(output_path, "combined_h_nh_col_pot_gum.xlsx")
# if (file.exists(exel_file))
#   file.remove(exel_file)
...
# #Delete file if it exists
# exel_file <- paste0(output_path, "combined_h_nh_col_pot_gpa.xlsx")
# if (file.exists(exel_file))
#   file.remove(exel_file)
...
# # #Delete file if it exists
# exel_file <- paste0(output_path, "combined_h_nh_col_pot_LN3.xlsx")
# if (file.exists(exel_file))
#   file.remove(exel_file)
...
# # #Delete file if it exists
# exel_file <- paste0(output_path, "combined_h_nh_col_pot_WEI.xlsx")
# if (file.exists(exel_file))
#   file.remove(exel_file)
...
# # #Delete file if it exists
# exel_file <- paste0(output_path, "combined_h_nh_col_pot_GP.xlsx")
# if (file.exists(exel_file))
#   file.remove(exel_file)
...
# 
# # #Delete file if it exists
exel_file <- paste0(output_path, "combined_h_nh_col_ym_GEV.xlsx")
if (file.exists(exel_file))
  file.remove(exel_file)
