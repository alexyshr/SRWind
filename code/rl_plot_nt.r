...
if (length(imp.vals$nt.series.dt) > 0) {
  ...
  #___     Domain in magnitude: y1: min (nt series), y2: max (nt series)
  ...
  #___   Write return values: zzz[zz,60:70]
  #___   Plot hazard curve!
  
  #Intensity Function Poisson Process. NIST.SP.500-301.pdf, page 29
    zeta = 0.00000001
    Ip.nt <- function(x) {1/z7 *(1+zeta*((x-z6)/z7))^((-1/zeta)-1)}
    #Ip.nt <- function(x) {1/z7 * exp(-(x-z6)/z7)}
    
    #Ant is same parameter Ans of equation before equation 4 (NIST.SP.500-301.pdf, page 16).
      #Here (ONLY non-thunderstorm) it is not (365- imp.vals$n.thunders.per.year*(1/24)) 
      #But it must be equal to ONE when in the dataset there is only NON-THUNDERSTORM data,
      # but Pintar station (722287) is working with 365 (BIG ISSUE TO SOLVE IN THESIS)
      #Meaning: Typical amount of non-thunderstorm in a year, but here always is ONE
      #         As "at" = 0, then ant = 365 -0 or ant = 1 -0
    #ant =  365 #If time is in days, but no match with other methods: 
               #  - Gumbel Quantile 
    ...
    ant = 1   #It does not mather if time is in days, but match with other methods:
               #  - Gumbel Quantile 
               #  - Formula Yn PP (Ecuation 18 pag 39 of NIST.SP.500-301.pdf)
               #  - Formula Yn GPD equivalent (Equation 15 pag 39 of NIST.SP.500-301.pdf)
    
    #ant = 0.999 #typical amount of non-thunderstorm in a year. 
  ...
  #Representing equation 4, page 16 (NIST.SP.500-301.pdf), but using Ip.nt
  ...
  #Probability density according to NIST.SP.500-301.pdf. Equation after equation 2, page 15
    densityPoissonProcess <- function (velocity, series, threshold){#, time){  #lower is the value of velocity to integrate with it up to infinite
      ...
      #                       units="days"))
      integrand <- Ip.nt
      #The domain in 'y' es from min to max 
      integrated <- integrate(integrand, lower=y1, upper=y2)$value
      #The domain in 'y' es from min to max, if change min by threshold is an error!
  ...
  #Plot dfP - probability density function using intensity function of Poisson Process
    ...
    #Plot PDFs for different shape values
    #Plot: Density Function from Intensity Function of Poisson Process (changing shape)
    #Plot dfP - probability density function using intensity function of Poisson Process (changing shape)
    # library(RColorBrewer)
    # cols <- brewer.pal(9,"Set1")
    # plot(x= yv, y=dfP, xlab="Declustered - Non-Thunderstorm - Velocities [Km/h]", ylab="Density Function - Poisson Process",
    #      main=paste("Density Function from Intensity Function of Poisson Process\n", "Location=", round(z6,2), "Scale=", round(z7,2), "Shape=[-0.5 to 0.5]", "\n",
    #                 "Station:", number, sep=" "),
    #      type="l", col=cols[3], lwd=4)
    # mtext(side = 1, text = paste0("Page ", numberofplots), outer = TRUE)
    # hist(imp.vals$nt.series, probability = TRUE, add=TRUE, col="cadetblue3")
    # lines(x= yv, y=dfP, col=cols[3], lwd=4)
    # #zetas = c(seq(-0.2, -0.01, by=0.01), seq(0.01, 0.5, by=0.01))
    # zetas = c(seq(-0.16, -0.1, by=0.01))
    # mycolors=terrain.colors(n=length(zetas))
    # myindex = 1
    # for (z in zetas){
    #   zeta = z
    #   dfP_zeta = sapply(yv, densityPoissonProcess, series=imp.vals$nt.series, threshold=z5)#, time=imp.vals$nt.series.dt)
    #   lines(x= yv, y=dfP_zeta, col=mycolors[myindex], lwd=0.5)
    #   myindex =+ 1
    # }  
    # zeta = 0.00000001
    # #assign(paste0("myprint", numberofplots), recordPlot())
  ...
  #       Be aware that the resulting value of dummy parameters is not affecting the final fitted GPD, because in equations: dummy/dummy
    ...
    #Fit equivalent GPD
      ...
      #ppcomp(fpnt)
      #qqcomp(fpnt)
      #quantile(fpnt, probs = c(0.05, 0.1, 0.2))
    # goodness-of-fit statistics
      mygof= fitdistrplus::gofstat(fpnt, discrete=FALSE)
    
    #Kolmogorov smirnov test
      mykstest = stats::ks.test(imp.vals$nt.series, y="pmygpd", location=gpdLocation, scale=gpdScale, shape=0.00001, dummy=1)

    # Plot equivalent POT-Poisson-GPD using fpnt
    # Four graphics:
    ...
    mtext(side = 1, text = paste0("Page ", numberofplots, " - Declustered - Non-Thunderstorm - POT-Poisson-GPD Equivalent. Location: ",
    ...
    # Write all results (GOF, KSTEST, RMSE, Parameters) to raw_data_station_id_fitted.xlsx (variable fnfitted)
      ...
      #write.xlsx(capture.output(myfitgpd), file=fnfitted, sheetName="nt_POT-Poisson-GPD-Equivalent", append=TRUE, row.names=TRUE)
      addWorksheet(fnfitted_OUT, "nt_POT-Poisson-GPD-Equivalent")
      writeData(fnfitted_OUT, sheet = "nt_POT-Poisson-GPD-Equivalent", x = capture.output(myfitgpd))
  ...
  #Calculate velocities for typical return periods using Poisson Process
    ...
    veocitiesfortypicalreturnperiodsP[is.na(veocitiesfortypicalreturnperiodsP)] <- 0
  #Alexys
    
    mat = cbind(paP=paP, returnperiods = 1/paP, yvels=yvels)
    mat2 = mat[order(paP),]
    write.table(mat2,file=paste0(outputpath, "hazard_curve_", number, ".csv"), sep=",",row.names=FALSE)
    
    mat3 = cbind(typicalExcedenceProbabilities=typicalExcedenceProbabilities,  returnperiods = 1/typicalExcedenceProbabilities, veocitiesfortypicalreturnperiodsP=veocitiesfortypicalreturnperiodsP)
    mat4 = mat3[order(typicalExcedenceProbabilities),]
    write.table(mat4,file=paste0(outputpath,"hazardcurve_importantpoints_", number, ".csv"), sep=",", row.names=FALSE)
    
  #Write return values to array zzz
    zzz[zz,60:70] = veocitiesfortypicalreturnperiodsP #Velocities for typical return periods Poisson
  ...
  #___   Return Values: NIST.SP.500-301.pdf, equation before equation 4 (page 16)
  #___     Ans (Ant) is 1
  #___   Write return values: zzz[zz,71:81]  
  #___   Plot hazard curve!
  #___
  ...
  #Intensity Function Poisson Process - Gumbel like tile length. Shape = 0. NIST.SP.500-301.pdf, page 32, equation 10
    ...
    #ant =  1 # It is not (365 - imp.vals$n.thunders.per.year*(1/24))  
                                                   #Same parameter Ans of equation before equation 4 (NIST.SP.500-301.pdf, page 16). 
                                                   #But it is equal to ONE when in the dataset there is only NON-THUNDERSTORM data
                                                   #Meaning: Typical amount of non-thunderstorm in a year, but here always is ONE
  ...
  #Calculate velocities for typical return periods
    ...
    #Replace NA with zero 
    veocitiesfortypicalreturnperiodsG[is.na(veocitiesfortypicalreturnperiodsG)] <- 0    
    zzz[zz,71:81] = veocitiesfortypicalreturnperiodsG #Velocities for typical return periods Gumbel Tile Length Intensity Function
  ...
  #___c) Using Gumbel Quantile replacing Poisson Process fitted parameters (location=z6, scale=z7)
  ...
  #        and write the return values to zzz[zz,82:92]
  ...
  #Replace NA with zero 
  veocitiesfortypicalreturnperiodsQG[is.na(veocitiesfortypicalreturnperiodsQG)] <- 0  
  ...
  #veocitiesfortypicalreturnperiodsQG = sapply(tipicalReturnPeriods, gumbelVelocitiesQuantileFunctionBerry, npy=npy, truncate=truncate)
    ...
    #Write the return values to the Excel file
    zzz[zz,82:92] = veocitiesfortypicalreturnperiodsQG #Velocities for typical return periods using Gumbel quantile function
